...
# Init
wgs = wgs_creator.LaWGS("debug")

# Airfoil
root_airfoil = wgs_creator.read_airfoil("mwe.csv", y_coordinate=0.5, expansion_ratio=1.5)
root_airfoil = root_airfoil.shift((11.4, 0., 0.))
tip_airfoil = root_airfoil.replace(x=12.9, y=1.5, z=0.)
wing = root_airfoil.linspace(tip_airfoil, num=30)
wgs.append_network("wing", wing, 1)

# Nose + Forebody
n_nose = 8  # number of points in the line
x_nose = np.linspace(0., 2.5, num=n_nose)  # x-coordinates of the line
...
fbody_p2 = fbody_p1.replace(x=11.4)
fbody_line = fbody_p1.linspace(fbody_p2, num=8)  # interpolate fbody_p1 and fbody_p2
...
aft_body_p2 = aft_body_p1.shift((0.5, -0.05, 0.))
...
body_base_line2 = body_base_line.replace(x=13.4, y=0., z=0.)  # create a `Line` with 13 identical points (the center of the circle)
...
wgs.create_stl()
wgs.create_wgs()
wgs.create_aux(alpha=0, mach=.7, cbar=2.3093, span=4., sref=6.928, xref=5.943, zref=0.)